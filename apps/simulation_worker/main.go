...
func performSim(region, realm, name string) (*[]byte, error) {
	...
	simCommand := exec.Command(SimcBinaryPath, fmt.Sprintf("armory=%v,%v,%v", region, realm, name))
...
func getSimcVersion() string {
	log.Print(SimcBinaryPath)
	simcCommand := exec.Command(SimcBinaryPath)
...
func main() {
	...
	go func() {
		for d := range msgChan {
			...
			_, err = queries.UpdateSimulation(
				context.Background(),
